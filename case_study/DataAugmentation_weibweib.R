da.discontinuation_weibweib<-function(theta, 
                             theta.prior, 
                             observed.data, 
                             ID.old, 
                             D.old){
  ...
  complete.data.old <- data.frame(#cens=observed.data$f.up, 
    Z=observed.data$Z,
    ...
    x2=observed.data$x2, 
    x3=observed.data$x3)
  complete.data.c   <- data.frame(#cens=observed.data$f.up, 
    Z=observed.data$Z,
  ...
  rr <- exp(logpost.i_weibweib(theta, theta.prior, complete.data.c) - 
              logpost.i_weibweib(theta, theta.prior, complete.data.old))
  ratio<-rep(0, n)
  ...
  # ratio[ii==1] <- ifelse(D.c[ii==1]<=Y[ii==1]/k,ratio[ii==1],0)
