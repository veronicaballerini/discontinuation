...
# Weibull-Weibull model: Weibull distributions for
# the potential time to discontinuation and the
# potential PFS
...
chain_weibweib <- mcmc.tdiscontinue_weibweib(niter = niter,
    burn = burn, thin = thin, par.start = par.start_weibweib,
    proposal = proposal_weibweib, theta.prior = theta.prior_weibweib,
    observed.data = observed.data)

# Exp-Weibull model: Exponential distribution for
# the potential time to discontinuation and Weibull
# distributions for the potential PFS
...
set.seed(474756)
chain_expweib <- mcmc.tdiscontinue_expweib(niter = niter,
    burn = burn, thin = thin, par.start = par.start_expweib,
    proposal = proposal_expweib, theta.prior = theta.prior_expweib,
    observed.data = observed.data)
#### Save results
save(chain_weibweib, chain_expweib, file = "other_models.RData")
...
THETA_weibweib <- rbind(chain_weibweib$Theta[seq(burn,
    niter, by = thin), ])
THETA_expweib <- rbind(chain_expweib$Theta[seq(burn, niter,
    by = thin), ])

# Parameters' mean
t_weibweib_mean <- as.matrix(apply(THETA_weibweib, 2,
    mean), ncol = 1)
t_weibweib_hdi <- as.matrix(t(apply(THETA_weibweib, 2,
    hdi)))
t_weibweib <- cbind(t_weibweib_mean, t_weibweib_hdi)
colnames(t_weibweib) <- c("posterior_mean", "HDI_lower",
    "HDI_upper")
saveRDS(t_weibweib, file = "tables/extra/t_weibweib.rds")

t_expweib_mean <- as.matrix(apply(THETA_expweib, 2, mean),
    ncol = 1)
t_expweib_hdi <- as.matrix(t(apply(THETA_expweib, 2, hdi)))
t_expweib <- cbind(t_expweib_mean, t_expweib_hdi)
colnames(t_expweib) <- c("posterior_mean", "HDI_lower",
    "HDI_upper")
...
for (i in 1:dim(THETA_weibweib)[2]) {
    jpeg(paste0("figures/trace/WeibWeib/", colnames(THETA_weibweib)[i],
...
for (i in 1:dim(THETA_expweib)[2]) {
    jpeg(paste0("figures/trace/ExpWeib/", colnames(THETA_weibweib)[i],
        ".jpeg"), width = 3500, height = 2500, units = "px",
        res = 300)
...
# Store principal strata membership posterior in
# lists
DID_weibweib <- array(NA, dim = c(dim(chain_weibweib$DID[[1]])[1],
    dim(chain_weibweib$DID[[1]])[2], dim(THETA_weibweib)[1]))
DID_expweib <- array(NA, dim = c(dim(chain_expweib$DID[[1]])[1],
    dim(chain_expweib$DID[[1]])[2], dim(THETA_expweib)[1]))
...
for (i in 1:dim(THETA_weibweib)[1]) {
    THETA_list_weibweib[[i]] <- list(eta.0 = THETA_weibweib[i,
        "eta.0"], eta.1 = THETA_weibweib[i, "eta.1"],
        eta.2 = THETA_weibweib[i, "eta.2"], eta.3 = THETA_weibweib[i,
            "eta.3"], alpha.d = THETA_weibweib[i, "alpha.d"],
        beta.d = THETA_weibweib[i, "beta.d"], eta.d1 = THETA_weibweib[i,
            "eta.d1"], eta.d2 = THETA_weibweib[i, "eta.d2"],
        eta.d3 = THETA_weibweib[i, "eta.d3"], alpha.y1nd = THETA_weibweib[i,
            "alpha.y1nd"], beta.y1nd = THETA_weibweib[i,
            "beta.y1nd"], alpha.y1d = THETA_weibweib[i,
            "alpha.y1d"], beta.y1d = THETA_weibweib[i,
            "beta.y1d"], alpha.y0nd = THETA_weibweib[i,
            "alpha.y0nd"], beta.y0nd = THETA_weibweib[i,
            "beta.y0nd"], alpha.y0d = THETA_weibweib[i,
            "alpha.y0d"], beta.y0d = THETA_weibweib[i,
            "beta.y0d"], eta.y1 = THETA_weibweib[i, "eta.y1"],
        eta.y2 = THETA_weibweib[i, "eta.y2"], eta.y3 = THETA_weibweib[i,
            "eta.y3"], delta = THETA_weibweib[i, "delta"],
        D = DID_weibweib[, , i][, 1], ID = DID_weibweib[,
            , i][, 2], data = observed.data)
}

THETA_list_expweib <- list()
for (i in 1:dim(THETA_expweib)[1]) {
    THETA_list_expweib[[i]] <- list(eta.0 = THETA_expweib[i,
        "eta.0"], eta.1 = THETA_expweib[i, "eta.1"], eta.2 = THETA_expweib[i,
        "eta.2"], eta.3 = THETA_expweib[i, "eta.3"], beta.d = THETA_expweib[i,
        "beta.d"], eta.d1 = THETA_expweib[i, "eta.d1"],
        eta.d2 = THETA_expweib[i, "eta.d2"], eta.d3 = THETA_expweib[i,
            "eta.d3"], alpha.y1nd = THETA_expweib[i, "alpha.y1nd"],
        beta.y1nd = THETA_expweib[i, "beta.y1nd"], alpha.y1d = THETA_expweib[i,
            "alpha.y1d"], beta.y1d = THETA_expweib[i,
            "beta.y1d"], alpha.y0nd = THETA_expweib[i,
            "alpha.y0d"], beta.y0nd = THETA_expweib[i,
            "beta.y0nd"], alpha.y0d = THETA_expweib[i,
            "alpha.y0d"], beta.y0d = THETA_expweib[i,
            "beta.y0d"], eta.y1 = THETA_expweib[i, "eta.y1"],
        eta.y2 = THETA_expweib[i, "eta.y2"], eta.y3 = THETA_expweib[i,
            "eta.y3"], delta = THETA_expweib[i, "delta"],
        D = DID_expweib[, , i][, 1], ID = DID_expweib[,
            , i][, 2], data = observed.data)
}
