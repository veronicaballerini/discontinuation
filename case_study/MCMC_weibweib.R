mcmc.tdiscontinue_weibweib <- function(niter, burn,
    ...
    repeat {
        theta.start <- list(eta.0 = rnorm(1, mean = par.start$mu.0,
            ...
            alpha.d = par.start$alpha.d + abs(rnorm(1,
                0, 0.1)), beta.d = par.start$beta.d +
                rnorm(1, 0, 0.1), eta.d1 = rnorm(1,
                mean = par.start$mu.d1, sd = par.start$sigma.d1),
            eta.d2 = rnorm(1, mean = par.start$mu.d2,
                sd = par.start$sigma.d2), eta.d3 = rnorm(1,
                mean = par.start$mu.d3, sd = par.start$sigma.d3),
            alpha.y1nd = par.start$shape.y1nd *
                par.start$scale.y1nd, beta.y1nd = rnorm(1,
            ...
            beta.y1d = rnorm(1, mean = par.start$mu.y1d,
                sd = par.start$sigma.y1d), alpha.y0nd = par.start$shape.y0nd *
                par.start$scale.y0nd, beta.y0nd = rnorm(1,
        ...
        D.start[ID.start == 0 & observed.data$Z ==
            0] <- rweibull(sum(ID.start == 0 & observed.data$Z ==
            0), shape = theta.start$alpha.d, scale = exp(-(theta.start$beta.d +
            ...
            0 & observed.data$Z == 0])/theta.start$alpha.d))
        ...
        theta <- list(eta.0 = theta.start$eta.0,
            ...
            eta.3 = theta.start$eta.3, alpha.d = theta.start$alpha.d,
            beta.d = theta.start$beta.d, eta.d1 = theta.start$eta.d1,
            eta.d2 = theta.start$eta.d2, eta.d3 = theta.start$eta.d3,
            alpha.y1nd = theta.start$alpha.y1nd,
            beta.y1nd = theta.start$beta.y1nd, alpha.y1d = theta.start$alpha.y1d,
            beta.y1d = theta.start$beta.y1d, alpha.y0nd = theta.start$alpha.y0nd,
            beta.y0nd = theta.start$beta.y0nd, alpha.y0d = theta.start$alpha.y0d,
        ...
        for (j in 1:niter) {
            ...
            discontinuedata <- da.discontinuation_weibweib(theta,
            ...
            log.post.eta.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.eta.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            ### alpha.d
            alpha.d <- rtgamma(1, shape = theta$alpha.d/proposal$scale.d,
                scale = proposal$scale.d, a = 1,
                b = Inf)

            thetaprop <- theta
            thetaprop$alpha.d <- alpha.d

            log.post.alpha.d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.alpha.d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)

            pr.num <- log.post.alpha.d.c + log(dtgamma(theta$alpha.d,
                shape = alpha.d/proposal$scale.d,
                scale = proposal$scale.d, a = 1,
                b = Inf))
            pr.den <- log.post.alpha.d.old + log(dtgamma(alpha.d,
                shape = theta$alpha.d/proposal$scale.d,
                scale = proposal$scale.d, a = 1,
                b = Inf))
            pr <- exp(pr.num - pr.den)

            u <- runif(1)
            theta$alpha.d <- ifelse((u <= pr & is.nan(pr) ==
                FALSE), alpha.d, theta$alpha.d)
            ...
            log.post.beta.d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.beta.d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            log.post.eta.d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.eta.d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            ### alpha.y1nd
            alpha.y1nd <- rgamma(1, shape = theta$alpha.y1nd/proposal$scale.y1nd,
                scale = proposal$scale.y1nd)

            thetaprop <- theta
            thetaprop$alpha.y1nd <- alpha.y1nd

            log.post.alpha.y1nd.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.alpha.y1nd.old <- logpost_weibweib(theta,
                theta.prior, complete.data)

            pr.num <- log.post.alpha.y1nd.c + log(dgamma(theta$alpha.y1nd,
                shape = alpha.y1nd/proposal$scale.y1nd,
                scale = proposal$scale.y1nd))
            pr.den <- log.post.alpha.y1nd.old +
                log(dgamma(alpha.y1nd, shape = theta$alpha.y1nd/proposal$scale.y1nd,
                  scale = proposal$scale.y1nd))

            pr <- exp(pr.num - pr.den)

            u <- runif(1)
            theta$alpha.y1nd <- ifelse((u <= pr &
                is.nan(pr) == FALSE), alpha.y1nd,
                theta$alpha.y1nd)
            ...
            log.post.beta.y1nd.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.beta.y1nd.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            ### alpha.y1d
            alpha.y1d <- rgamma(1, shape = theta$alpha.y1d/proposal$scale.y1d,
                scale = proposal$scale.y1d)

            thetaprop <- theta
            thetaprop$alpha.y1d <- alpha.y1d

            log.post.alpha.y1d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.alpha.y1d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            u <- runif(1)
            theta$alpha.y1d <- ifelse((u <= pr &
                is.nan(pr) == FALSE), alpha.y1d,
                theta$alpha.y1d)
            ...
            log.post.beta.y1d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.beta.y1d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            ### alpha.y0nd
            alpha.y0nd <- rgamma(1, shape = theta$alpha.y0nd/proposal$scale.y0nd,
            ...
            log.post.alpha.y0nd.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.alpha.y0nd.old <- logpost_weibweib(theta,
                theta.prior, complete.data)

            pr.num <- log.post.alpha.y0nd.c + log(dgamma(theta$alpha.y0nd,
                shape = alpha.y0nd/proposal$scale.y0nd,
                scale = proposal$scale.y0nd))
            pr.den <- log.post.alpha.y0nd.old +
                log(dgamma(alpha.y0nd, shape = theta$alpha.y0nd/proposal$scale.y0nd,
                  scale = proposal$scale.y0nd))
            pr <- exp(pr.num - pr.den)

            u <- runif(1)
            theta$alpha.y0nd <- ifelse((u <= pr &
                is.nan(pr) == FALSE), alpha.y0nd,
                theta$alpha.y0nd)
            ...
            log.post.beta.y0nd.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.beta.y0nd.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            ### alpha.y0d
            alpha.y0d <- rgamma(1, shape = theta$alpha.y0d/proposal$scale.y0d,
                scale = proposal$scale.y0d)

            thetaprop <- theta
            thetaprop$alpha.y0d <- alpha.y0d

            log.post.alpha.y0d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.alpha.y0d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            theta$alpha.y0d <- ifelse((u <= pr &
                is.nan(pr) == FALSE), alpha.y0d,
                theta$alpha.y0d)
            ...
            log.post.beta.y0d.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.beta.y0d.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            log.post.eta.y.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.eta.y.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
            ...
            log.post.delta.c <- logpost_weibweib(thetaprop,
                theta.prior, complete.data)
            log.post.delta.old <- logpost_weibweib(theta,
                theta.prior, complete.data)
