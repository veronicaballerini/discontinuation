rmste_apply_expexp <- function(x){
  ...
  for (j in seq_along(d)) {
    ...
    for (h in seq_along(y)) {
      ...
      surv1 <- Stexp(
        h,
        rate = exp(x$beta.y1d + linpred_y + x$delta * log_j),
        a = j
      )
      ...
      surv0 <- Sexp(
        h,
        # x$alpha.y0d,
        exp(x$beta.y0d + linpred_y + x$delta * log_j)
      )
  ...
  DCE_D <- colSums(DCE_Y_D * matrix(fD_av, 
                                    nrow = length(d), ncol = length(y), byrow = FALSE))
