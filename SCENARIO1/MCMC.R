mcmc.tdiscontinue <- function(niter, burn, thin, par.start, 
  ...
  theta.start <- list(
    ...
    eta.3     = rnorm(1, mean  = par.start$mu.3, sd = par.start$sigma.3),
    ...
    eta.d3     = rnorm(1, mean  = par.start$mu.d3, sd = par.start$sigma.d3),
    ...
    eta.y3     = rnorm(1, mean  = par.start$mu.y3, sd = par.start$sigma.y3),
  ...
  pi         = exp(theta.start$eta.0+
                     ...
                     theta.start$eta.3*observed.data$x3)/(1+
    ...
    rweibull(sum(ID.start==0 & observed.data$Z==0), 
             ...
             scale=exp(-(theta.start$beta.d+
                           ...
                           theta.start$eta.d2*observed.data$x2[ID.start==0 & 
                                                                 observed.data$Z==0]+
                           theta.start$eta.d3*observed.data$x3[ID.start==0 & 
  ...
  theta <- list(eta.0=theta.start$eta.0,eta.1=theta.start$eta.1,eta.2=theta.start$eta.2,eta.3=theta.start$eta.3,
                alpha.d=theta.start$alpha.d,beta.d=theta.start$beta.d,
                eta.d1=theta.start$eta.d1,eta.d2=theta.start$eta.d2,eta.d3=theta.start$eta.d3,
                ...
                eta.y1=theta.start$eta.y1,eta.y2=theta.start$eta.y2,eta.y3=theta.start$eta.y3,
  ...
  for(j in 1:niter){
    ...
    ###eta1
    eta.1<-rnorm(1,theta$eta.1,proposal$sd.1)
    ...
    ###eta2
    eta.2<-rnorm(1,theta$eta.2,proposal$sd.2)
    ...
    ###eta3
    eta.3<-rnorm(1,theta$eta.3,proposal$sd.3)
    ...
    ###eta.d2
    eta.d2 <- rnorm(1,theta$eta.d2, proposal$sd.d2)
    ...
    ###eta.d3
    eta.d3 <- rnorm(1,theta$eta.d3, proposal$sd.d3)
    ...
    rm(eta.d3, thetaprop, u, pr.num, pr.den, pr, log.post.eta.d.c, log.post.eta.d.old)
    ...
    #y1
    eta1 <-  rnorm(1,theta$eta.y1, proposal$sd.y1)
    ...
    #y2
    eta2 <-  rnorm(1,theta$eta.y2, proposal$sd.y2)
    ...
    #y3
    eta3 <-  rnorm(1,theta$eta.y3, proposal$sd.y3)
    ...
    rm(thetaprop, eta3, u, pr.num, pr.den, pr, log.post.eta.y.c, log.post.eta.y.old)
    
    ###lambda
    lambda <-  rnorm(1,theta$lambda, proposal$sd.lambda)
