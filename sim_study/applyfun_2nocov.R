applyfun_2nocov <- function(x) {

    # set the seed
    seed <- x

    # if this seed has already generated a dataset, use that dataset
    if (file.exists(paste(wd, "/sim/simdata2_", seed, ".txt", sep = ""))) {
        set.seed(seed)
    } else {
        ...
        # Pre-treatment variables: X1 = Continuous variable; the bigger the number, the higher the risk (Age) X2 = Presence of specific metastatic status
        # before enrolling the study X3 = Disease burden indicator. Treatment variable: Z=z, z=0,1 Discontinuation indicator: ID = 1 if the discontinuation
        # cannot be observed (due to death or progression) ID = 0 if the discontinuation is observed, even after the follow-up period Discontinuation variable:
        # D1 = Continuous variable; time until the discontinuation under active treatment Outcome variable: Y = survival
        ...
        Y0[ID == 0] <- rtweibull(sum(1 - ID), shape = theta$alpha.y0d, scale = scale.y0d[ID == 0], a = D1[ID == 0])
        ...
        save(thetatrue, file = paste(wd, "/sim/thetatrue2_", seed, ".RData", sep = ""))
        rm(theta)

        realdata <- data.frame(Z = Z, Y1 = Y1, Y0 = Y0, x1 = x1st, x2 = x2, x3 = x3, ID = ID, D = D1)
        write.table(realdata, paste(wd, "/sim/realdata2_", seed, ".txt", sep = ""))

        write.table(data, paste(wd, "/sim/simdata2_", seed, ".txt", sep = ""))
    ...
    # upload the data
    observed.data <- read.table(paste(wd, "/sim/simdata2_", seed, ".txt", sep = ""), header = T)

    # set the seed for the chain
    set.seed(474747)

    # run the chain
    chain_nocov <- mcmc.tdiscontinue_nocov(niter, burn, thin, par.start, theta.prior, observed.data[, -c(10:13)], scenario = 2, seed = seed)
    ...
    if (is.na(chain_nocov$nojump) == FALSE) {
        ...
        realdata <- read.table(paste(wd, "/sim/realdata2_", seed, ".txt", sep = ""))
        thetatrue <- myload(paste(wd, "/sim/thetatrue2_", seed, ".RData", sep = ""))
        rdce <- dce(thetatrue, d = seq(1, 5), y = seq(1, 20), realdata, scenario = 2)
